...
# function to verify the Collatz conjecture holds true for a positice integer 
def collatz(number):
  collatz_sequence_list=[]
  wrong_list=[]
  loop_list = [4, 2, 1]
  if number == 1:
    collatz_sequence_list = [1 ,4, 2, 1] # to test 1 the 4,2,1 loop needs to be included to test against
  elif number == 2:
    collatz_sequence_list = [2, 4, 2, 1] # to test 2 the 4,2,1 loop needs to be included to test against
  else:
...
def result(lowest_number, highest_number):
  ...
  if len(wrong_list_var) > 0:
    ...
    print(f'The Collatz Conjecture holds true for the nubers from {lowest_number} up to and including {highest_number-1}')
